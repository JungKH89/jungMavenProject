  ...
  def main(args: Array[String]): Unit = {
    ...
    var groupRdd1 = mapRdd.
      ...
      map(x=>{
        ...
        if(data.size == 0){
          ...
          avg =Math.round((data.map(x => {x.getDouble(qtyNo)}).sum) / data.size)
          //avg = (data.map(x => {x.getDouble(qtyNo)}).sum) / data.size
        }
    ...
    var groupRdd2 = mapRdd.
      ...
      flatMap(x=> {
        ...
        var avg = 1.0d
        var size = data.size
        var sum = (data.map(x => {x.getDouble(qtyNo)}).sum)
        if (data.size == 0) {
          avg = 0
        } else {
    ...
    var  finalResult = spark.sql("SELECT REGIONID, PRODUCT, SUBSTR(YEARWEEK,5,2) AS WEEK, ROUND(AVG(RATIO),2) AS AVG_RATIO FROM MIDDLETABLE "
